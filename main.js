...
let inclinaisonAxeRotationDiv = document.querySelector(
  ".inclinaison-axe-rotation-div"
);
...
const envMap = await hdrLoader.loadAsync(
  "/Rendu-workshop-3js/public/textures/galaxie.hdr"
);
...
//parametres pour calculer la trajectoire de la camera à la transition
...
//création de la légende des planetes
 ...
 */
  ...
  planets = [
...

    {
      ...
      normalMapImage:
        "/Rendu-workshop-3js/public/texture/2k_earth_normal_map.tif",
      lunesACreer: 1,
      lunes: [
        {
          nom: "Lune",
  ...
  const circleTexture = new THREE.TextureLoader().load(
    "/Rendu-workshop-3js/public/textures/circle.png"
  );
  ...
  const sunTexture = new THREE.TextureLoader().load(
    "/Rendu-workshop-3js/public/textures/sunmap.jpg"
  );
  ...
  planets.forEach((orbit) => {
    ...
    orbit.lunes?.forEach((lune) => {
      const moonGeometry = new THREE.SphereGeometry(lune.moonRadius, 16, 8);
      ...
      moonMesh.position.set(
        planet.planetPoint.x + lune.moonDistance,
        0,
        planetPoint.y
      );
      scene.add(moonMesh);
  ...
  if (flyAnim) {
    ...
    if (t < 1) {
      camera.position.lerpVectors(
        flyAnim.startPosition,
        flyAnim.targetPosition,
        ease
      );
      // console.log(flyAnim.startPosition, flyAnim.targetPosition);
      controls.target.lerpVectors(
        flyAnim.startTarget,
        flyAnim.targetCenter,
        ease
      );
